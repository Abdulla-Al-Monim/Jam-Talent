...
class PagesController extends Controller
{
    ...
    {
        $user = User::find(1);
        $countFreelancer = User::orderBy('hourly_rate','desc')->where('user_type_id',1)->where('verified',1)->whereNotNull('status')->count();
        $highRatedUsers = User::orderBy('hourly_rate','desc')->where('user_type_id',1)->where('verified',1)->whereNotNull('status')->get();
        return view('frontend.pages.index',compact('highRatedUsers','countFreelancer'));
    }
    ...
    public function knowledgeHub()
    {
        ...
        return view('frontend.pages.knowledgeHub',compact('blogs','blogsPaginate','trandPosts','blogsCount'));
