...
class BlogController extends Controller
{
    ...
    {
        $blogs = Blog::where('type',$name)->where('status',1)->get();
        $blogsCount = Blog::where('type',$name)->where('status',1)->count();
        ...
        if ($blogsCount == 0) {
            ...
            $blogsPaginate = Blog::where('status',1)->paginate(5);
            $trandPosts = Blog::where('status',1)->limit(3)->get();
        }
