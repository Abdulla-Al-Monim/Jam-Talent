...
class OrderController extends Controller
{
    ...
    public function freelancerActiveOrder()
    {
        if (Auth::user()->user_type_id == 1) {
            ...
            $orders = Order::orderBy('id','desc')->where('freelancer_id',Auth::user()->id)->where('status',1)->paginate(5);
    ...
    public function freelancerDeliveredOrder()
    {
        if (Auth::user()->user_type_id == 1) {
            $user = User::Find(Auth::user()->id);
            $orders = Order::orderBy('id','desc')->where('freelancer_id',Auth::user()->id)->where('status',2)->paginate(5);
    ...
    public function freelancerCompletedOrder()
    {
        if (Auth::user()->user_type_id == 1) {
            $user = User::Find(Auth::user()->id);
            $orders = Order::orderBy('id','desc')->where('freelancer_id',Auth::user()->id)->where('status',3)->paginate(5);
    ...
    //place order
    public function placeOrderFreelancer(Request $request, $id)
    {
        ...
        $delivaryOrder->save();
        $order = Order::where('id',$delivaryOrder->order_id)->first();
        $order = Order::Find($order->id);
        $order->status = 2;
        $order->save();
        return redirect()->route('freelancer.delivered.order');
    ...
    public function manageCancelRequectFreelancer()
    {
        if (Auth::user()->user_type_id == 1) 
        {
        $orderCancels = Order::orderBy('id','desc')->where('status',5)->where('freelancer_id',Auth::user()->id)->paginate(5);
    ...
    public function manageCancelOrderFreelancer()
    {
        if (Auth::user()->user_type_id == 1) 
        {
        $orderCancels = Order::orderBy('id','desc')->where('status',6)->where('freelancer_id',Auth::user()->id)->paginate();
    ...
    public function manageOrderEmp()
    {
        if (Auth::user()->user_type_id == 3) {
            $user = User::Find(Auth::user()->id);
        $orders = Order::orderBy('id','desc')->where('employer_id',Auth::user()->id)->where('status',1)->paginate(5);
    ...
    public function manageDeliveredOrderEmp()
    {
        if (Auth::user()->user_type_id == 3) {
            $user = User::Find(Auth::user()->id);
        $orders = Order::orderBy('id','desc')->where('employer_id',Auth::user()->id)->where('status',2)->paginate(5);
    ...
    public function manageConfirmedOrderEmp()
    {
        if (Auth::user()->user_type_id == 3) {
            $user = User::Find(Auth::user()->id);
            $orders = Order::orderBy('id','desc')->where('employer_id',Auth::user()->id)->where('status',3)->paginate(5);
     ...
     public function confirmOrderEmployer(Request $request, $id,$orderId)
    {
        if (Auth::user()->user_type_id == 3) {
            ...
            $freelancerBalance = new FreelancerBalance;
            $freelancerBalance->order_id = $orderId;
            $freelancerBalance->user_id = $order->freelancer_id;
            $freelancerBalance->balace = $order->budget;
            $freelancerBalance->save();

            return redirect()->route('order.confirmed.emp');
        }
        else{
            return view('frontend.pages.pages-404');
        }
    ...
    public function manageCancelRequectEmployer()
    {
        if (Auth::user()->user_type_id == 3) 
        {
        $orderCancels = Order::orderBy('id','desc')->where('status',5)->where('employer_id',Auth::user()->id)->paginate(5);
    ...
    public function manageCancelOrderEmployer()
    {
        if (Auth::user()->user_type_id == 3) 
        {
        $orderCancels = Order::orderBy('id','desc')->where('status',6)->where('employer_id',Auth::user()->id)->paginate(5);
    ...
    public function allCancelOrder()
    {
        if (Auth::user()->user_type_id == 2) {
            $user = User::Find(Auth::user()->id);
        $orders = Order::orderBy('id','desc')->where('status',6)->get();
    ...
    public function allActiveOrderLaslWeeek()
    {
        ...
        $user = User::Find(Auth::user()->id);
        $orders = Order::orderBy('id','desc')->where('status',1)->where('created_at','>=',$date)->get();
