...
class ReviewController extends Controller
{
    ...
    public function reviewManageFreelancer()
    {
        if (Auth::user()->user_type_id == 1) {
            $reviewFreelancers = ReviewFreelancer::orderBy('id','asc')->where('user_id',Auth::user()->id)->paginate(5);
            $reviewEmployers = ReviewEmployer::orderBy('id','asc')->where('freelancer_id',Auth::user()->id)->paginate(5);
    ...
    public function reviewOrderEmployer(Request $request, $id, $freelancerActivityId)
    {
        ...
        $review->on_budget          = $request->radio;
        $review->on_time            = $request->radio2;
        $review->rating             = $request->rating;
        $review->comment            = $request->message;
        $review->status             = 1;
        $review->save();
    ...
    public function reviewManageEmployer()
    {
        if (Auth::user()->user_type_id == 3) {
            $reviewFreelancers = ReviewFreelancer::orderBy('id','asc')->where('employer_id',Auth::user()->id)->paginate(5);
            $reviewEmployers = ReviewEmployer::orderBy('id','asc')->where('user_id',Auth::user()->id)->paginate(5);
