...
class OrderCancelController extends Controller
{
    ...
    {
        $orderCancels = OrderCancel::orderBy('id','asc')->where('status',0)->where('cancel_request',1)->get();
        return view('backend.pages.admin.task.cancelRequestFreelancer',compact('orderCancels'));
    }
    //confirm cancel reqaust freelancer
     public function confirmCancelRequestFreelancer($orderId,$id)
    {
    ...
    {
        $orderCancels = OrderCancel::orderBy('id','asc')->where('status',0)->where('cancel_request',2)->get();
