...
class EmployerController extends Controller
{
    ...
    {
        $emp = User::Find($id);
        return view('frontend.pages.employer.profile',compact('emp'));
    }
    ...
    public function update(Request $request)
    {
        ...
        $employer->location = $request->nationality;
        $full_name = $request->first_name . ' ' . $request->last_name;
        $employer->full_name            = $full_name;
        if( is_null($employer->user_name))
        {
            
            $full_name = str_replace(' ','',$full_name);
            $full_name                  = $full_name . rand(0,100);
            $employer->user_name                = $full_name;
            $employer->gender                = $request->gender;
            $employer->slug                     = Str::slug($full_name); 
        }
        else
        {
